...
def show_rounded_popup(parent, anchor_widget, options, variable, on_select=None):
    ...
    Args:
        parent: The main window (used for bindings and as toplevel parent)
        anchor_widget: The button that triggered the popup
        options: List of string options to display
        variable: tk.StringVar to update on selection
        on_select: Optional callback(value) after selection
    """
...
def show_manage_models_dialog(gui):
    ...
    Args:
        gui: The LocatorGUI instance (provides ALL_MODELS, quality_options,
             quality_var, status_var, and model management methods)
    """
